...
## Das Problem mit n=1: Nur zwei Datenpunkte
...
ggplot(data_observed, aes(x = Behandlung, y = Ertrag)) +
  ...
  labs(title = "Beobachtete Werte (n=1 pro Behandlung)",
       x = "Behandlung",
       y = "Ertrag [dt/ha]") +
  theme_BioMath(base_family = "Arial")
```

## Was wir nicht sehen: Die mögliche Population
...
# Simulierte Daten: Beide Behandlungen kommen aus Population mit Mittelwert 9
...
ggplot() +
  ...
  scale_fill_identity() +
  scale_y_continuous(limits = c(0, 13), breaks = seq(0, 13, 2)) +
  labs(title = "Was wir nicht wissen: Mögliche Population",
       subtitle = "Graue Punkte: Simulierte Werte aus Population mit gleichem Mittelwert (μ=9)",
       x = "Behandlung",
       y = "Ertrag [dt/ha]") +
  theme_BioMath(base_family = "Arial")
```
...
::: {.fragment}
**Kernproblem:** Mit nur n=1 könnten beide Werte zufällig aus derselben Population stammen!
:::
